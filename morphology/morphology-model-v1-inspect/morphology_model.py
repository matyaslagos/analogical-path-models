...
def produce_word_inspected(model, lemma, target_tag_set):
    """Produce a word form and report the substitutions that built it.

    Mirrors the scoring of `inflect`, but additionally records, for every
    candidate word form, each attested ending substitution that contributed to
    its goodness score. Returns the breakdown for the winning word form.

    Returns a dict with:
        - 'produced word': the guessed word form (decoded), or '' if the guess
          fails (no candidates, or a tie for first place, as in `produce_word`)
        - 'score': goodness score of the winning word form
        - 'contributions': list of the substitutions that produced the winning
          word form, sorted by reliability (highest first). Each item is a dict:
            - 'tag': the analogical (source) tag whose forms were used
            - 'substitution': '<old ending> -> <new ending>' (decoded)
            - 'reliability': P_{t->g}(e -> e'), the substitution's reliability
            - 'attested by': list of (lemma, source-tag form, target-tag form)
              triples (decoded) attesting this substitution
    """
    ...
    # For each candidate word form, the contributing substitutions
    contributions = defaultdict(list)
    decode_suffix = lambda s: cio.hun_decode(s) or '∅'
    for anl_tag, anl_lemmas in bases.items():
        transform_dict = defaultdict(Counter)
        # Which analogical lemmas attest each (old suffix -> new suffix)
        ...
        for anl_lemma in anl_lemmas:
            ...
            for base_suffix in base_suffixes:
                ...
                attestations[(base_suffix, new_suffix)].append(
                    (anl_lemma, anl_lemma_wordform, anl_lemma_target_wordform)
                )
        # Record reliabilities and the substitutions that produced each form
        for original_suffix, new_suffixes in transform_dict.items():
            ...
            for new_suffix, new_suffix_count in new_suffixes.items():
                ...
                contributions[new_wordform].append({
                    'tag': set(anl_tag),
                    'substitution': '{} -> {}'.format(
                        decode_suffix(original_suffix), decode_suffix(new_suffix)
                    ),
                    'reliability': prob,
                    'attested by': [
                        (cio.hun_decode(al), cio.hun_decode(aw), cio.hun_decode(atw))
                        for al, aw, atw in attestations[(original_suffix, new_suffix)]
                    ],
                })
    best_wordforms = sorted(new_wordforms.items(), key=itemgetter(1), reverse=True)
    if not best_wordforms:
        return {'produced word': '', 'score': 0.0, 'contributions': []}
    best_wordform, best_score = best_wordforms[0]
    # Mirror produce_word: a tie for first place counts as a failed guess
    tie = len(best_wordforms) > 1 and best_wordforms[0][1] == best_wordforms[1][1]
    produced_word = '' if tie else cio.hun_decode(best_wordform)
    best_contributions = sorted(
        contributions[best_wordform], key=itemgetter('reliability'), reverse=True
    )
    return {
        'produced word': produced_word,
        'score': best_score,
        'contributions': best_contributions,
    }
